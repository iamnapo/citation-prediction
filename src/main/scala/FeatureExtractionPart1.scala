...
object FeatureExtractionPart1 {
  ...
  def main(args: Array[String]) {
    ...
    val ss = SparkSession.builder().appName("Feature Extraction Part 1").master("local[*]").getOrCreate()
    ...
    def getPagerank = udf((id: String) => {
      val pagerank = pageRank.filter(_._1.toString == id).map(_._2)
      var rank_1 = 0.0
      if (pagerank.length > 0) rank_1 = pagerank.take(1)(0)
      rank_1
    })

    post_training_set_df = post_training_set_df
      .withColumn("source_pagerank", getPagerank($"Source"))
      .withColumn("target_pagerank", getPagerank($"Target"))
